...
class ChatsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(create: (_) => ChatsTabViewModel(), child: const _ChatsTabView());
  }
}
...
class _ChatsTabView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer3<ChatsTabViewModel, VendorStoreContextViewModel, VendorPreviewSessionViewModel>(
      builder: (context, viewModel, storeContext, previewSession, _) {
        ...
        final threads = viewModel.filteredThreads
            .where((thread) => visibleOrderServices.contains(thread.serviceType))
            .toList();
                ...
                SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      ...
                      _FilterChip(
                        label: 'Customers',
                        selected: viewModel.counterpartFilter == VendorChatCounterpartType.customer,
                        color: colors.serviceFood,
                        onTap: () => viewModel.setCounterpartFilter(VendorChatCounterpartType.customer),
                      ),
                      _FilterChip(
                        label: 'Riders',
                        selected: viewModel.counterpartFilter == VendorChatCounterpartType.rider,
                        color: colors.serviceGrabMart,
                        onTap: () => viewModel.setCounterpartFilter(VendorChatCounterpartType.rider),
                      ),
                      _FilterChip(
                        label: 'Unread',
                ...
                if (visibleOrderServices.isEmpty)
                  ...
                  ...threads.map(
                    (thread) => _ChatThreadTile(
                      ...
                      onOpenOrder: () => _openOrder(context, viewModel, thread.orderId),
                      timeLabel: viewModel.relativeTimeLabel(thread.lastMessageAt),
                    ),
                  ),
              ],
            ),
  ...
  void _openThread(BuildContext context, ChatsTabViewModel viewModel, String threadId) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => ChatThreadPage(threadId: threadId, parentViewModel: viewModel),
      ),
    );
  }

  void _openOrder(BuildContext context, ChatsTabViewModel viewModel, String orderId) {
    final order = viewModel.linkedOrder(orderId);
    if (order == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Linked order details are unavailable')));
      return;
    }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: colors.backgroundPrimary,
          borderRadius: BorderRadius.circular(14.r),
          border: Border.all(color: thread.isAtRisk ? colors.error.withValues(alpha: 0.35) : colors.border),
        ),
        child: Column(
          children: [
            Row(
                ...
                Column(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      icon: Icon(Icons.open_in_new_rounded, size: 18.sp, color: colors.vendorPrimaryBlue),
            ...
            Row(
              children: [
                ...
                if (thread.isAtRisk) ...[SizedBox(width: 6.w), _Tag(label: 'At Risk', color: colors.error)],
                if (thread.hasOpenIssue) ...[SizedBox(width: 6.w), _Tag(label: 'Issue', color: colors.warning)],
              ],
            ),
          ],
        ),
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: color.withValues(alpha: 0.14), borderRadius: BorderRadius.circular(999.r)),
      child: Text(
        label,
