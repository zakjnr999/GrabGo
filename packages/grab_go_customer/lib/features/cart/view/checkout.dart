...
class _CheckoutState extends State<Checkout> with TickerProviderStateMixin {
                                ...
                                Padding(
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      if (promoDiscount > 0) ...[
                                        SizedBox(height: 6.h),
                                        _buildPriceRow(context, "Promo Discount", -promoDiscount, colors, false, false),
                                      ],
                                      if (creditsApplied > 0) ...[
                                        SizedBox(height: 6.h),
                                        _buildPriceRow(
  ...
  void _showPaymentConfirmationSheet(BuildContext context, CartProvider provider, AppColorsExtension colors) {
    ...
    final double serviceFee = provider.serviceFee;
    final double originalDeliveryFee = isPickupMode ? 0 : provider.originalDeliveryFee;
    final double originalServiceFee = provider.originalServiceFee;
    final double rainFee = isPickupMode ? 0 : provider.rainFee;
    final double subscriptionDeliveryDiscount = isPickupMode ? 0 : provider.subscriptionDeliveryDiscount;
    final double subscriptionServiceFeeDiscount = provider.subscriptionServiceFeeDiscount;
    final double creditsApplied = _effectiveCreditsAppliedForCheckout(provider, isPickupMode: isPickupMode);
    final double effectiveTip = isPickupMode ? 0.0 : _tipAmount;
    final double total = _checkoutGrandTotal(provider, isPickupMode: isPickupMode);
  ...
  Widget _buildDiscountedFeeAmount({
    ...
    if (isLoading) {
      return Text(
        "...",
        style: TextStyle(color: colors.textSecondary, fontSize: compact ? 12.sp : 13.sp, fontWeight: FontWeight.w500),
      );
    }
    ...
    final Widget text = RichText(
      text: TextSpan(
        children: [
          TextSpan(text: "GHS ${resolvedOriginalAmount.toStringAsFixed(2)}", style: oldStyle),
          TextSpan(
            text: " / ",
            style: oldStyle.copyWith(decoration: TextDecoration.none),
          ),
          TextSpan(text: amount <= 0 ? "FREE" : "GHS ${amount.toStringAsFixed(2)}", style: newStyle),
        ],
      ),
    );
        ...
        child: Container(
          ...
          child: Row(
            children: [
              ...
              _buildAnimatedRadioIndicator(isSelected: isSelected, colors: colors, outerSize: 24.h, innerSize: 10.h),
            ],
          ),
        ),
      ),
  ...
  Widget _buildAnimatedRadioIndicator({
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        color: isSelected ? colors.accentOrange.withValues(alpha: activeAlpha) : Colors.transparent,
      ),
      child: Center(
        child: AnimatedScale(
          duration: const Duration(milliseconds: 220),
