...
class OrderDetailSheet extends StatelessWidget {
  ...
  const OrderDetailSheet({super.key, required this.detail, required this.formatDate});
  @override
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _detailLine(context, 'Payment Method', detail.paymentMethod ?? 'n/a'),
            _detailLine(context, 'Payment Provider', detail.paymentProvider ?? 'n/a'),
            _detailLine(
              context,
              'Payment Ref',
              detail.paymentReferenceId == null || detail.paymentReferenceId!.isEmpty
                  ? 'n/a'
                  : detail.paymentReferenceId!,
            ),
            _detailLine(context, 'Total', '${detail.currency} ${detail.totalAmount.toStringAsFixed(2)}'),
            ...
            _detailLine(context, 'Scheduled Pickup', formatDate(detail.scheduledPickupAt)),
            const Divider(height: 30),
            const Text('Stops & Contacts', style: TextStyle(fontWeight: FontWeight.w700)),
            SizedBox(height: 8.h),
            ...
            _detailLine(context, 'Description', detail.packageDescription ?? 'n/a'),
            _detailLine(context, 'Size Tier', detail.sizeTier),
            _detailLine(context, 'Weight', '${detail.weightKg.toStringAsFixed(2)} kg'),
            _detailLine(context, 'Declared Value', '${detail.currency} ${detail.declaredValueGhs.toStringAsFixed(2)}'),
            ...
            const Divider(height: 30),
            const Text('Return-to-Sender Financials', style: TextStyle(fontWeight: FontWeight.w700)),
            SizedBox(height: 8.h),
            _detailLine(context, 'Return Charge Status', detail.returnChargeStatus),
            _detailLine(
              context,
            ...
            const Divider(height: 30),
            const Text('Recent Events', style: TextStyle(fontWeight: FontWeight.w700)),
            SizedBox(height: 8.h),
                    ...
                    (event) => Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(event.eventType, style: const TextStyle(fontWeight: FontWeight.w600)),
                            SizedBox(height: 4.h),
                            Text('Actor: ${event.actorRole ?? "n/a"}'),
                            Text('Time: ${formatDate(event.createdAt)}'),
                            if (event.reason != null && event.reason!.isNotEmpty) Text('Reason: ${event.reason}'),
                            if (event.metadata.isNotEmpty)
                              Text(
                                'Metadata: ${event.metadata}',
                                style: TextStyle(color: context.appColors.textSecondary, fontSize: 12.sp),
                              ),
                          ],
                        ),
                      ),
