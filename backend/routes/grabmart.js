...
/**
 * @route   GET /api/grabmart/recommended
 * @desc    Get personalized GrabMart recommendations (ML-first with heuristic fallback)
 * @access  Public (Optional Auth)
 */
router.get(
    "/recommended",
    optionalAuth,
    cacheMiddleware(`${cache.CACHE_KEYS.GRABMART}:recommended`, 180, true),
    async (req, res) => {
        try {
            ...
            if (userId) {
                try {
                    const mlRecommendations = await mlClient.getStoreRecommendations(userId, 'grabmart', 30);
                    ...
                    if (recommendedStoreIds.length > 0) {
                        ...
                        if (eligibleStoreIds.length > 0) {
                            ...
                            const candidateItems = await prisma.grabMartItem.findMany({
                                where: {
                                    isAvailable: true,
                    ...
                    console.error('🤖 GrabMart ML recommendation failed, using fallback:', mlError.message);
                }
            }
            ...
            if (finalRecommendations.length < limit) {
                ...
                const fallbackPool = await prisma.grabMartItem.findMany({
                    where: {
                        isAvailable: true,
            ...
            console.error('Get GrabMart recommended items error:', error);
